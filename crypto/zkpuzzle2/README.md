...
## Summary
Challenge 2 is essentially challenge 1 but with some extra constraints.

First the server requires p1 != p2 and
```python
ROUND = ROUND - bin(p1).count("1") - bin(p2).count("1")
```
We need to pick p1, p2 so that the resulting ROUND count still lets us reach the 941-success threshold to obtain the flag.

## Exploit
Two issues remain:
* Find p1, p2
* Find w; each component must now be < 400 bits, a looser bound than in challenge 1

First, I found a pair p1, p2 that works:
```bash 
...
Next we need the CRT solution $d = r k^3 \ mod \ orderE_1$ and $d = r k^3 \ mod \ orderE_2$, then factorize d and distribute factors into a, b, c as in challenge 1.
...
if __name__ == "__main__":
    ...
    io = remote('instance.penguin.0ops.sjtu.cn', 18192)
...
```
flag: 0ctf{A_g0Od_pUzZ1e_C0n5tra1ns_7he_WitNess_N0t_thE_proof!!~~~}
```
